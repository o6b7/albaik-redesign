...
export function Header() {
  ...
  return (
    <View className="flex-row justify-between bg-gray-100 rounded-md mb-2 items-center" style={{ zIndex: 50 }}>
      <View>
        <Text className='mb-1'>{greeting}</Text>
        <View className="flex-row items-center gap-2">
          <TouchableOpacity onPress={() => setShowDropDown(!showDropDown)} className="flex-row items-center gap-2">
            <Text className="text-xl font-bold text-gray-600">{user?.fullName ?? 'Guest'}</Text>
            <ChevronDown size={16} color="#718096" />
          </TouchableOpacity>
          {showDropDown && (
            <View className='absolute top-8 left-32 bg-white rounded-md shadow-md p-2 z-50' style={{ elevation: 5 }}>
