  ...
  const renderItem = ({ item }: { item: CartItem }) => (
    <View className='flex-row items-center bg-gray-50 rounded-2xl p-3 mb-3'>
      ...
      <View className='items-center gap-2'>
        <View className='flex-row items-center border border-gray-300 rounded-full px-3 py-1 gap-3'>
          <Pressable onPress={() => updateQuantity(item.id, item.quantity - 1)}
            className="disabled:opacity-50"
          >
            <Minus size={14} color="#111" />
          </Pressable>
          <Text className='text-gray-900 font-bold text-sm w-4 text-center'>{item.quantity}</Text>
          <Pressable onPress={() => updateQuantity(item.id, item.quantity + 1)}>
  ...
  if (items.length === 0) {
    return (
      <SafeAreaView className='flex-1 items-center justify-center bg-white'>
        <Text className='text-gray-400 text-lg'>You cart is empty</Text>
      </SafeAreaView>
    )
  }
